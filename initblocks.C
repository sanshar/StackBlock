						 ...
						 const int& restartSize, const bool &restart, const bool& warmUp, int integralIndex, const vector<SpinQuantum>& braquanta, const vector<SpinQuantum>& ketquanta)
{
  ...
  else if (forward)
  {
    ...
    {
      startingBlock = StackSpinBlock(0, forward_starting_size - 1, integralIndex, startingBlock.nonactive_orb(), leftState==rightState, true);
    }
    else
    ...
    if (dmrginp.add_noninteracting_orbs() && dmrginp.molecule_quantum().get_s().getirrep() != 0 && dmrginp.spinAdapted())
    {
      ...
      newstartingBlock.nonactive_orb() = startingBlock.nonactive_orb();
...
void SpinAdapted::InitBlocks::InitBigBlock(StackSpinBlock &leftBlock, StackSpinBlock &rightBlock, StackSpinBlock &big, const std::vector<SpinQuantum>& braquanta, const std::vector<SpinQuantum>& ketquanta)
{
    ...
    if (dmrginp.warmup() == LOCAL2 || dmrginp.warmup() == LOCAL3 || dmrginp.warmup() == LOCAL4) {
      ...
      if (coreSites.size() > 0) {
        ...
        if ((!dot_with_sys && onedot) || !onedot) {
          ...
          environment.default_op_components(!forward, leftState == rightState);
          environment.setstoragetype(DISTRIBUTED_STORAGE);
          environment.BuildSumBlock(constraint, environmentActive, environmentCore, braquanta, ketquanta);
        } else {
	        newEnvironment.set_integralIndex() = integralIndex;
          newEnvironment.default_op_components(direct, haveNormops, haveCompops, leftState == rightState);
          newEnvironment.setstoragetype(DISTRIBUTED_STORAGE);
          newEnvironment.BuildSumBlock(constraint, environmentActive, environmentCore, braquanta, ketquanta);
	        //p2out << "\t\t\t NewEnvironment block " << endl << newEnvironment << endl;
	        //newEnvironment.printOperatorSummary();
        }
      } else { // no core
