      ...
      //for (int tcount=0;tcount<leftBlock->get_op_array(CRE).size();tcount++){
        ...
        for (int ucount=0;ucount<rightBlock->get_op_array(CRE).size();ucount++){
          ...
          for (int iquanta=0;iquanta<VQ.size();iquanta++){
            ...
            boost::shared_ptr<Wavefunction> VtuT(new Wavefunction(VQ[iquanta],&big,true));
            //multiply the wavefunction with the triplet operator
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*lop,Transposeview(*rop),&big,WF,*Vtu,opQ,1.0);
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*lop),*rop,&big,WF,*VtuT,opQ,-1.0);
            //------------------
            //store the operator
            //------------------
            VtuArray[iquanta+1]->AppendOperator(Vtu,t,u);
          ...
          boost::shared_ptr<Wavefunction> VtuSingT(new Wavefunction(VQ[0],&big,true));
          //multiply the wavefunction with the triplet operator
          operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*lop,Transposeview(*rop),&big,WF,*VtuSing,opQ,1.0);
          operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*lop),*rop,&big,WF,*VtuSingT,opQ,1.0);
          //------------------
          //store the operator
          //------------------
          VtuArray[0]->AppendOperator(VtuSing,t,u);
