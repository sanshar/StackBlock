  ...
  void CheckOperator_(SpinBlock &big, Wavefunction &WF, int t, int u, int v, SparseMatrix &Op, bool left){
    ...
    for (int i=0;i<leftBlock->get_op_array(CRE).size();i++){
      ...
      if (left){
        ...
        operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*tmpop),Op,&big,WF,*tmpwf,totOpQ[0],sqrt(2.0));
        //build the element
    ...
    for (int i=0;i<rightBlock->get_op_array(CRE).size();i++){
      ...
      if (!left){
        ...
        vector<SpinQuantum> totOpQ = opQ - tmpop->get_deltaQuantum(0);
        boost::shared_ptr<Wavefunction> tmpwf(new Wavefunction(WFQ,&big,true));
        operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(rightBlock,Transposeview(*tmpop),Op,&big,WF,*tmpwf,totOpQ[0],sqrt(2.0));
        //build the element
        double val = DotProduct(WF,*tmpwf);
        sprintf(msg,"\nCheckOperator: D(%i,%i,%i,%i)=%4.8e  left a",t,u,v,v_,val);
        pout << msg;
      ...
      for (int vcount=0;vcount<rightBlock->get_op_array(CRE).get_size();vcount++){
        ...
        for (int iquanta=0;iquanta<VQ.size();iquanta++){
          ...
          if (!DummyIter){
            ...
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*LeftOpSing,Transposeview(*RightOp),&big,WF,*VtuvS,VopQS[0],SingFac[iquanta]);
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*LeftOpTrip,Transposeview(*RightOp),&big,WF,*VtuvT,VopQS[0],TripFac[iquanta]);
      ...
      for (int tu=0;tu<Dim_tu;tu++){
        ...
        for (int vcount=0;vcount<leftBlock->get_op_array(CRE).get_size();vcount++){
          ...
          for (int iquanta=0;iquanta<VQ.size();iquanta++){
            ...
            //multiply the Wavefunction with the operators
            if (!DummyIter){
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOp),*RightOpSing,&big,WF,*VtuvS,VopQS[0],SingFac[iquanta]);
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOp),*RightOpTrip,&big,WF,*VtuvT,VopQS[0],TripFac[iquanta]);
              //sum up the contributions
              ScaleAdd(1.0,*VtuvS,*Vtuv);
              ScaleAdd(1.0,*VtuvT,*Vtuv);
            }
      ...
      for (int ucount=0;ucount<rightBlock->get_op_array(CRE).get_size();ucount++){
        ...
        for (int iquanta=0;iquanta<VQ.size();iquanta++){
          ...
          if (t!=v){
            ...
            if (!DummyIter){
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOpSing),*RightOp,&big,WF,*VvtuS,VopQS[0],1.0);
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOpTrip),*RightOp,&big,WF,*VvtuT,VopQS[0],1.0);
              //sum up the contributions
      ...
      for (int tv=0;tv<Dim_tv;tv++){
        ...
        for (int ucount=0;ucount<leftBlock->get_op_array(CRE).get_size();ucount++){
          ...
          for (int iquanta=0;iquanta<VQ.size();iquanta++){
            ...
            if (t!=v){
              if (!DummyIter){
                operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*LeftOp,Transposeview(*RightOpSing),&big,WF,*VvtuS,VopQS[0],1.0);
                operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*LeftOp,Transposeview(*RightOpTrip),&big,WF,*VvtuT,VopQS[0],1.0);
                ScaleAdd(SingFac[iquanta],*VvtuS,*Vvtu);
                ScaleAdd(-TripFac[iquanta],*VvtuT,*Vvtu);//account for transposition and add a minus sign
              }
            }//t!=u
    ...
    for (int uv=0;uv<Dim_uv;uv++){
      ...
      for (int tcount=0;tcount<rightBlock->get_op_array(CRE).get_size();tcount++){
        ...
        for (int iquanta=0;iquanta<VQ.size();iquanta++){
          ...
          if (!DummyIter){
            //multiply the Wavefunction with the operators
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOpSing),*RightOp,&big,WF,*VtuvS,VopQS[0],SingFac[iquanta]);
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOpTrip),*RightOp,&big,WF,*VtuvT,VopQS[0],TripFac[iquanta]);
            //sum up the contributions
            ScaleAdd(1.0,*VtuvS,*Vtuv);
            ScaleAdd(1.0,*VtuvT,*Vtuv);
          }
      ...
      for (int uv=0;uv<Dim_uv;uv++){
        ...
        for (int tcount=0;tcount<leftBlock->get_op_array(CRE).get_size();tcount++){
          ...
          for (int iquanta=0;iquanta<VQ.size();iquanta++){
            ...
            //multiply the Wavefunction with the operators
            if (!DummyIter){
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(rightBlock,Transposeview(*RightOpSing),*LeftOp,&big,WF,*VtuvS,VopQS[0],SingFac[iquanta]);
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(rightBlock,Transposeview(*RightOpTrip),*LeftOp,&big,WF,*VtuvT,VopQS[0],TripFac[iquanta]);
              //sum up the contributions
              ScaleAdd(1.0,*VtuvS,*Vtuv);
              ///+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
              // NOTE:
    ...
    for (int tv=0;tv<Dim_tv;tv++){
      ...
      for (int ucount=0;ucount<rightBlock->get_op_array(CRE).get_size();ucount++){
        ...
        for (int iquanta=0;iquanta<VQ.size();iquanta++){
          ...
          if (!DummyIter){
            //multiply the Wavefunction with the operators
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*LeftOpSing,Transposeview(*RightOp),&big,WF,*VtvuS,VopQS[0],1.0);
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*LeftOpTrip,Transposeview(*RightOp),&big,WF,*VtvuT,VopQS[0],1.0);
            //sum up the contributions
            ScaleAdd(SingFac[iquanta],*VtvuS,*Vtvu);
            ScaleAdd(TripFac[iquanta],*VtvuT,*Vtvu);
          }
      ...
      for (int tv=0;tv<Dim_tv;tv++){
        ...
        for (int ucount=0;ucount<leftBlock->get_op_array(CRE).get_size();ucount++){
          ...
          for (int iquanta=0;iquanta<VQ.size();iquanta++){
            ...
            if (!DummyIter){
              //multiply the Wavefunction with the operators
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(rightBlock,*RightOpSing,Transposeview(*LeftOp),&big,WF,*VtvuS,VopQS[0],1.0);
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(rightBlock,*RightOpTrip,Transposeview(*LeftOp),&big,WF,*VtvuT,VopQS[0],1.0);
              //sum up the contributions
              ScaleAdd(SingFac[iquanta],*VtvuS,*Vtvu);
              ScaleAdd(TripFac[iquanta],*VtvuT,*Vtvu);
            }
            ...
            if (t!=v){
              if (!DummyIter){
                operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOp),Transposeview(*RightOpSing),&big,WF,*VvtuS,VopQS[0],1.0);
                operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*LeftOp),Transposeview(*RightOpTrip),&big,WF,*VvtuT,VopQS[0],1.0);
                ScaleAdd(SingFac[iquanta],*VvtuS,*Vvtu);
                ScaleAdd(-TripFac[iquanta],*VvtuT,*Vvtu);//account for transposition and add a minus sign
              }
            }//t!=u
