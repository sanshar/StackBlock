  ...
  void generate_terminal_replacement_operators(const SpinBlock& big, Wavefunction& WF,
    ...
    for (li = li_start; li < li_stop; li++) {
      ...
      for (ri = 0; ri < rightBlock->get_op_array(CRE).get_size(); ri++) {
        ...
        operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, *lop, Transposeview(*rop), &big, WF, *B_x_psi, opQ[0], sqrt(2.0));
        //store the product
        T.AppendOperator(B_x_psi, j, i);
        //do the same for the transposed matrix 
        boost::shared_ptr<Wavefunction> B_x_psi_T(new Wavefunction(dQ, &big, true));
        operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, Transposeview(*lop), *rop, &big, WF, *B_x_psi_T, opQ[0], sqrt(2.0));
          ...
          for (n=0;n<norbs;n++) {
            ...
            for (int kcount=k0;kcount < k1;kcount++){
              ...
              for (int lcount = 0; lcount < rightBlock->get_op_array(CRE).get_size(); lcount++) {
                ...
                boost::shared_ptr<Wavefunction> tmp(new Wavefunction(dQ, &big, true));
                //multiply the linking operator with the right terminal operator
                operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, *lop, Transposeview(*rop), &big, *right_op, *tmp, opQ, sqrt(2.0));
                //form the dot product of the resulting wavefunction and the left terminal operator
                val = DotProduct(*left_op, *tmp);
                //add the result to the three particle density matrix (it is the leading three-body term)
                threepdm(i,k,m,j,l,n) += val;
                ...
                if (k!=l){
                  tmp->Clear();
                  operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, Transposeview(*lop), *rop, &big, *right_op, *tmp, opQ, sqrt(2.0));
                  val = DotProduct(*left_op, *tmp);
                  threepdm(i,l,m,j,k,n) += val;
                  threepdm_C(i,l,m,j,k,n) += val;
                }//k!=l
      ...
      for (j=0;j<norbs;j++){
        ...
        for (int kcount=k0;kcount<k1;kcount++){
          ...
          for (int lcount=0;lcount<rightBlock->get_op_array(CRE).get_size();lcount++){
            ...
            boost::shared_ptr<Wavefunction> tmp(new Wavefunction(dQ, &big, true));
            //multiply the linking operator with the right terminal operator
            operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, *lop, Transposeview(*rop), &big, *terminal, *tmp, opQ, sqrt(2.0));
            //store the resulting wavefunction
            T_IM.AppendOperator(tmp,aji,alk);
            //do the same for the transposed operator
            if (k!=l){
              boost::shared_ptr<Wavefunction> tmp_T(new Wavefunction(dQ, &big, true));
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, Transposeview(*lop), *rop, &big, *terminal, *tmp_T, opQ, sqrt(2.0));
              T_IM.AppendOperator(tmp_T,aji,akl);
            }//k!=l
          }//l
        }//k
        ...
        for (k=0;k<norbs;k++){
          ...
          for (icount=i0;icount<i1;icount++){
            ...
            for (jcount=0;jcount<rightBlock->get_op_array(CRE).get_size();jcount++){
              ...
              boost::shared_ptr<Wavefunction> tmp_T2(new Wavefunction(dQ, &big, true));
              //multiply the linking operator with the terminal operator
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*lop,Transposeview(*rop),&big,*terminal1,*tmp1,opQ,sqrt(2.0));
              if (i!=j)operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*lop),*rop,&big,*terminal1,*tmp_T1,opQ,sqrt(2.0));
              operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,*lop,Transposeview(*rop),&big,*terminal2,*tmp2,opQ,sqrt(2.0));
              if (i!=j)operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock,Transposeview(*lop),*rop,&big,*terminal2,*tmp_T2,opQ,sqrt(2.0));
              //sum the product of operator strings and integrals
