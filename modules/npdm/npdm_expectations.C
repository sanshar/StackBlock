...
//FIXME is null_deleter() necessary below?
  ...
  boost::shared_ptr<StackSparseMatrix> lhsOp, dotOp, rhsOp;
  ...
  if ( (lhsOps.opReps_.size() > 0) && (dotOps.opReps_.size() > 0) && (rhsOps.opReps_.size() > 0) ) {
    StackTransposeview lhsOpTr = StackTransposeview(*lhsOp);
    if ( lhsOps.transpose_ ) lhsOp = boost::shared_ptr<StackSparseMatrix>( &lhsOpTr, boostutils::null_deleter() );
    StackTransposeview dotOpTr = StackTransposeview(*dotOp);
    if ( dotOps.transpose_ ) dotOp = boost::shared_ptr<StackSparseMatrix>( &dotOpTr, boostutils::null_deleter() );
    StackTransposeview rhsOpTr = StackTransposeview(*rhsOp);
    if ( rhsOps.transpose_ ) rhsOp = boost::shared_ptr<StackSparseMatrix>( &rhsOpTr, boostutils::null_deleter() );
...
double Npdm_expectations::build_nonspin_adapted_singlet_expectations( NpdmSpinOps_base & lhsOps, NpdmSpinOps_base & rhsOps, NpdmSpinOps_base & dotOps)
{
  ...
  {
    std::map<std::vector<int>, StackWavefunction> leftwaves;
    std::map<std::vector<int>, StackWavefunction> rightwaves;
    ...
    spin.push_back(0);
    spin.push_back(!rhsOps.transpose_? (-rhsOps.opReps_.at(0)->get_deltaQuantum(0).get_s()).getirrep(): rhsOps.opReps_.at(0)->get_deltaQuantum(0).get_s().getirrep());
    StackWavefunction& lw= leftwaves.at(spin);
    StackWavefunction& rw= rightwaves.at(std::vector<int>(1,0));
      ...
      for (int ilhs = 0; ilhs < std::max(1,hilhs); ++ilhs) {
        ...
        if ( singlet ) 
        {
          if(dmrginp.npdm_intermediate() && (npdm_order_== NPDM_NEVPT2 || npdm_order_== NPDM_THREEPDM || npdm_order_== NPDM_FOURPDM))
          {
            ...
            {
              assert(hirhs>0);
              StackWavefunction& rw= rightwaves.at(std::vector<int>(1,irhs));
              expectations_[omprank].push_back(DotProduct_spincorrection(lw,rw,big_));
            }
          }
          else
            expectations_[omprank].push_back( contract_spin_adapted_operators( ilhs, idot, irhs, lhsOps, rhsOps, dotOps) ); }
      }
    }
  }

  assert (expectations_[omprank].size() > 0);
//pout << "---------------------------------\n";
...
Npdm_expectations::get_nonspin_adapted_expectations( NpdmSpinOps_base & lhsOps, NpdmSpinOps_base & rhsOps, NpdmSpinOps_base & dotOps )
{
  ...
  spin_adaptation_.npdm_set_up_linear_equations(dim, op_string, indices, expectations_[omprank], A, b, so_indices );
...
std::vector< std::pair< std::vector<int>, double > > 
Npdm_expectations::get_nonspin_adapted_expectations(NpdmSpinOps_base & lhsOps, NpdmSpinOps_base & rhsOps, NpdmSpinOps_base & dotOps, std::map<std::vector<int>, StackWavefunction>& leftwaves, std::map<std::vector<int>, StackWavefunction>& rightwaves )
{
  // Initialize dimension of spin-adapted to non-spin-adapted transformation
  int dim = 0;
     ...
     // int ds= hidot? dotOps.opReps_.at(idot)->get_deltaQuantum(0).get_s().getirrep(): 0;
      ...
      if((!dmrginp.spinAdapted()? std::abs(ls+ds): std::abs(ls-ds)) <= rindices )
      {
        boost::shared_ptr<StackSparseMatrix> lhsOp, dotOp;
        {
          ...
          if(lhsOps.transpose_) lhsOp=boost::shared_ptr<StackSparseMatrix>(new StackTransposeview(*lhsOps.opReps_.at(ilhs)));
          else lhsOp= lhsOps.opReps_.at(ilhs);
        }

        {
          assert(dotOps.opReps_.size() > 0);
          if(dotOps.transpose_) dotOp=boost::shared_ptr<StackSparseMatrix>(new StackTransposeview(*dotOps.opReps_.at(idot)));
	  ...
	  vector<SpinQuantum> spins = (dotOp->get_deltaQuantum(0) + lhsOp->get_deltaQuantum(0));
	  SpinQuantum AOpq;
	  for (int i=0; i< spins.size(); i++) {
	    if (spins[i].get_s().getirrep() == total_spin) { AOpq = spins[i]; break; }
	  }

          StackWavefunction opw2;
          opw2.initialise(dQ[0]-AOpq, big_.get_leftBlock()->get_ketStateInfo(),big_.get_rightBlock()->get_braStateInfo(), true);
	  opw2.Clear();
