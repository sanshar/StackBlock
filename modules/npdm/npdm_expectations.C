  ...
  if(!dmrginp.spinAdapted()){
   ...
   // pout <<"size of operator: " <<lhsOps.opReps_.size()<<','<<rhsOps.opReps_.size()<<','<<dotOps.opReps_.size()<<endl;
      ...
      int ds= !dotOps.transpose_? dotOps.opReps_.at(0)->get_deltaQuantum(0).get_s().getirrep(): (-dotOps.opReps_.at(0)->get_deltaQuantum(0).get_s()).getirrep();
          ...
          operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(big_.get_leftBlock(), AOp, *rhsOp, &big_, wavefunction_0, opw_0,dQ[0], lhsOps.factor_*dotOps.factor_*rhsOps.factor_);
...
void Npdm_expectations::restore( NpdmSpinOps_base & lhsOps, NpdmSpinOps_base & dotOps, std::map<std::vector<int>, Wavefunction>& halfwaves)
{
#ifndef SERIAL
  boost::mpi::communicator world;
#endif
...
void Npdm_expectations::restore( NpdmSpinOps_base & rhsOps, std::map<std::vector<int>, Wavefunction>& halfwaves)
{
#ifndef SERIAL
  boost::mpi::communicator world;
#endif
