...
void spinExpectation(StackWavefunction& wave1, StackWavefunction& wave2, SparseMatrix& leftOp, SparseMatrix& dotOp, SparseMatrix& rightOp, const SpinBlock& big, vector<double>& expectations, bool doTranspose)
{
  ...
  { 
    operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, AOp, rightOp, &big, wave2, opw2, dQ[0], 1.0);
    expectations.push_back( DotProduct(wave1, opw2, dmrginp.Sz(), big) );    
    if (doTranspose)
    {
      opw2.Clear();
      operatorfunctions::TensorMultiply<SpinBlock, SparseMatrix, SparseMatrix>(leftBlock, Transposeview(AOp), rightOp, &big, wave2, opw2, dQ[0], 1.0);
      expectations.push_back( DotProduct(wave1, opw2, dmrginp.Sz(), big) );    
    }
  }
  opw2.deallocate();
