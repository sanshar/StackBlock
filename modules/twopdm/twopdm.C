...
namespace SpinAdapted{
  void compute_twopdm_sweep(std::vector<StackWavefunction>& wavefunctions, const SpinBlock& system, const SpinBlock& systemDot, const SpinBlock& newSystem, const SpinBlock& newEnvironment, const SpinBlock& big, const int numprocs, int state)
{
  ...
  array_4d<double> twopdm(big.size()*2, big.size()*2, big.size()*2, big.size()*2);
	...
	int i = state;
	int j = i;
	StackWavefunction &wavefunction1 = wavefunctions[0];
	StackWavefunction &wavefunction2 = wavefunctions[0];
	...
	if (dmrginp.setimplicitTranspose())
	  compute_two_pdm_1_2_1(wavefunction1, wavefunction2, big, twopdm);
	else
	  compute_two_pdm_1_2_1_notranspose(wavefunction1, wavefunction2, big, twopdm);
	...
	if (dmrginp.setimplicitTranspose())
	  ...
	  compute_two_pdm_1_1_2_notranspose(wavefunction1, wavefunction2, big, twopdm);
...
void compute_twopdm_final(std::vector<StackWavefunction>& wavefunctions, const SpinBlock& system, const SpinBlock& systemDot, const SpinBlock& newSystem, const SpinBlock& newEnvironment, const SpinBlock& big, const int numprocs, int state)
{
  const int nroots = wavefunctions.size();
  array_4d<double> twopdm(big.size()*2, big.size()*2, big.size()*2, big.size()*2);
...
void compute_twopdm_initial(std::vector<StackWavefunction>& wavefunctions, const SpinBlock& system, const SpinBlock& systemDot, const SpinBlock& newSystem, const SpinBlock& newEnvironment, const SpinBlock& big, const int numprocs, int state)
{
  const int nroots = wavefunctions.size();
  array_4d<double> twopdm(big.size()*2, big.size()*2, big.size()*2, big.size()*2);
  //for(int i=0;i<nroots;++i)
	...
	int i = state;
	int j = i;
	StackWavefunction &wavefunction1 = wavefunctions[0];//there is only one wavefunction in the vector the 
	StackWavefunction &wavefunction2 = wavefunctions[0];
	load_twopdm_binary(twopdm, i ,j);
	...
	if (dmrginp.setimplicitTranspose())
	  ...
	  compute_two_pdm_2_1_1_notranspose(wavefunction1, wavefunction2, big, twopdm);
...
void compute_two_pdm_0_4_0(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  ...
  if (!dmrginp.setimplicitTranspose()) {
    boost::shared_ptr<SparseMatrix> dotop1 = dotBlock->get_op_rep(DES_DES, -sq0, dotindex, dotindex);
    operatorfunctions::Product(dotBlock, *dotop0, *dotop1, dotop, -1.0);
  }
  else 
...
void compute_two_pdm_4_0_0(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  ...
  if (!dmrginp.setimplicitTranspose()) {
    boost::shared_ptr<SparseMatrix> leftop1 = leftBlock->get_op_rep(DES_DES, -sq0, leftindex, leftindex);
    operatorfunctions::Product(leftBlock, *leftop0, *leftop1, leftop, -1.0);
  }
  else
...
void compute_two_pdm_0_0_4(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  ...
  if (!dmrginp.setimplicitTranspose()) {
    boost::shared_ptr<SparseMatrix> rightop1 = rightBlock->get_op_rep(DES_DES, -sq0, rightindex, rightindex);
    operatorfunctions::Product(rightBlock, *rightop0, *rightop1, rightop, -1.0);
  }
  else
...
void compute_two_pdm_0_2_2(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
  ...
  for (int ij = 0; ij < dotBlock->get_op_array(CRE_CRE).get_size(); ++ij)
  {
	...
	}
	{
	  ...
	  sq0 = (getSpinQuantum(kx)+getSpinQuantum(lx))[0];//(SpinQuantum(1,1,SymmetryOfSpatialOrb(kx))+SpinQuantum(1,1,SymmetryOfSpatialOrb(lx)))[0];
	  boost::shared_ptr<SparseMatrix> rightop0 = rightBlock->get_op_rep(CRE_CRE, sq0, kx, lx);
	  spinExpectation(wave1, wave2, *leftOp, *dotop0, *rightop0, big, expectations, false);
	  spinExpectation(wave1, wave2, *leftOp, *dotop2, *rightop2, big, expectations, false);
	  
	  vector<int> indices(4,0);
	  indices[0] = lx; indices[1] = kx; indices[2] = ix; indices[3] = jx;
	  //expectations[0]*=-1; expectations[1]*=-1;
	  spin_to_nonspin(indices, expectations, twopdm, CC_DD, false);
	}
      }
  ...
  for (int ij = 0; ij < dotBlock->get_op_array(CRE_DES).get_size(); ++ij)
  {
      ...
      else {
	{
	  ...
	  int kx = rightop2->get_orbs(0);
	  int lx = rightop2->get_orbs(1);
	  boost::shared_ptr<SparseMatrix> rightop0 = rightBlock->get_op_array(CRE_DES).get_local_element(kl)[0]->getworkingrepresentation(rightBlock);
	  spinExpectation(wave1, wave2, *leftOp, *dotop0, *rightop0, big, expectations, false);
	  spinExpectation(wave1, wave2, *leftOp, *dotop2, *rightop2, big, expectations, false);
	...
	{
	  boost::shared_ptr<SparseMatrix> dotop2 = dotBlock->get_op_array(DES_CRE).get_local_element(ij)[1]->getworkingrepresentation(leftBlock);
	  int ix = dotop2->get_orbs(0);
	  int jx = dotop2->get_orbs(1);
	  boost::shared_ptr<SparseMatrix> dotop0 = dotBlock->get_op_array(DES_CRE).get_local_element(ij)[0]->getworkingrepresentation(leftBlock);
	  ...
	  int kx = rightop2->get_orbs(0);
	  int lx = rightop2->get_orbs(1);
	  boost::shared_ptr<SparseMatrix> rightop0 = rightBlock->get_op_array(DES_CRE).get_local_element(kl)[0]->getworkingrepresentation(rightBlock);
	  spinExpectation(wave1, wave2, *leftOp, *dotop0, *rightop0, big, expectations, false);
	  spinExpectation(wave1, wave2, *leftOp, *dotop2, *rightop2, big, expectations, false);
...
void compute_two_pdm_2_0_2(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock()->get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
  ...
  for (int ij = 0; ij < cdsize; ++ij)
  {
	...
	int ix = leftop2->get_orbs(0);
	int jx = leftop2->get_orbs(1);
	boost::shared_ptr<SparseMatrix> leftop0 = leftBlock->get_op_array(CRE_DES).get_local_element(ij)[0]->getworkingrepresentation(leftBlock);
	SparseMatrix* dotop = 0;
	...
	int ix = leftop2->get_orbs(0);
	int jx = leftop2->get_orbs(1);
	boost::shared_ptr<SparseMatrix> leftop0 = leftBlock->get_op_array(DES_CRE).get_local_element(ij)[0]->getworkingrepresentation(leftBlock);
	SparseMatrix* dotop = 0;
...
void compute_two_pdm_2_2_0(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock()->get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  ...
  for (int ij = 0; ij < cdsize; ++ij)
  {
	...
	for (int kl =0; kl <dotBlock->get_op_array(CRE_DES).get_size(); kl++)
	{
	  boost::shared_ptr<SparseMatrix> dotop2 = dotBlock->get_op_array(DES_CRE).get_local_element(kl)[1]->getworkingrepresentation(dotBlock);
	  int kx = dotop2->get_orbs(0);
	  int lx = dotop2->get_orbs(1);
	  boost::shared_ptr<SparseMatrix> dotop0 = dotBlock->get_op_array(DES_CRE).get_local_element(kl)[0]->getworkingrepresentation(dotBlock);//dotBlock->get_op_rep(CRE_DES_S0, kx, lx);
	  vector<double> expectations;
	  spinExpectation(wave1, wave2, *leftop0, *dotop0, *rightop, big, expectations, false);
	  spinExpectation(wave1, wave2, *leftop2, *dotop2, *rightop, big, expectations, false);
...
void compute_two_pdm_1_1_2(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
...
void compute_two_pdm_1_1_2_notranspose(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
  ...
  for (int j = 0; j < leftBlock->get_leftBlock()->get_op_array(CRE).get_size(); ++j)
  {
	...
	vector<int> indices(4,0);
	vector<double> expect1(2), expect2(2);
	expect1[0] = expectations[0]; expect1[1] = expectations[1];
	indices[0] = ix; indices[1] = kx; indices[2] = jx; indices[3] = lx;
	spin_to_nonspin(indices, expect1, twopdm, CD_CD, false); 
	

	expectations.clear();
	leftop = leftBlock->get_leftBlock()->get_op_array(DES).get_local_element(j)[0];
	dotop  = dotBlock->get_op_rep(CRE, getSpinQuantum(dotindex) , dotindex);
	spinExpectation(wave1, wave2, *leftop, *dotop, *rightop0, big, expectations, false);
	spinExpectation(wave1, wave2, *leftop, *dotop, *rightop2, big, expectations, false);
	
	expect2[0] = expectations[0]; expect2[1] = -expectations[1];
	indices[0] = jx; indices[1] = kx; indices[2] = ix; indices[3] = lx;
	spin_to_nonspin(indices, expect2, twopdm, CD_CD, false);  
	...
	if (lx != kx) {
	  ...
	  expect1[0] = expectations[0]; expect1[1] = expectations[1];
	  indices[0] = ix; indices[1] = kx; indices[2] = jx; indices[3] = lx;
	  spin_to_nonspin(indices, expect1, twopdm, CD_D_C, false); //**************
	  ...
	  expect2[0] = expectations[0]; expect2[1] = -expectations[1];
	  indices[0] = jx; indices[1] = kx; indices[2] = ix; indices[3] = lx;
	  spin_to_nonspin(indices, expect2, twopdm, CD_D_C, false);  //**************
	}
      }
...
void compute_two_pdm_1_2_1(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
...
void compute_two_pdm_2_1_1(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock()->get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = big.get_leftBlock()->get_rightBlock();
...
void compute_two_pdm_1_2_1_notranspose(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
...
void compute_two_pdm_2_1_1_notranspose(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock()->get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = big.get_leftBlock()->get_rightBlock();
...
void compute_two_pdm_0_3_1(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
...
void compute_two_pdm_3_0_1(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock()->get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = big.get_leftBlock()->get_rightBlock();
...
void compute_two_pdm_3_1_0(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
...
void compute_two_pdm_1_3_0(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
...
void compute_two_pdm_1_3(StackWavefunction& wave1, StackWavefunction& wave2, const SpinBlock& big, array_4d<double>& twopdm)
{
  SpinBlock* leftBlock = big.get_leftBlock();
  SpinBlock* rightBlock = big.get_rightBlock();
  SpinBlock* dotBlock = leftBlock->get_rightBlock();
