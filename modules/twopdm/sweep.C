...
namespace SpinAdapted{
  void SweepTwopdm::BlockAndDecimate (SweepParams &sweepParams, SpinBlock& system, SpinBlock& newSystem, const bool &useSlater, const bool& dot_with_sys, int stateA, int stateB)
{
  ...
  InitBlocks::InitNewSystemBlock(system, systemDot, newSystem, stateA, stateB, sweepParams.get_sys_add(), dmrginp.direct(), system.get_integralIndex(), DISTRIBUTED_STORAGE, true, true);
  
  InitBlocks::InitNewEnvironmentBlock(environment, systemDot, newEnvironment, system, systemDot, stateA, stateB, 
  ...
  solution[0].Clear();
  //******************************
  //GuessWave::guess_wavefunctions(solution[0], e, big, sweepParams.get_guesstype(), true, stateA, true, 0.0); 
  ...
  if (sweepParams.get_block_iter() == 0)
    compute_twopdm_initial(solution, system, systemDot, newSystem, newEnvironment, big, numprocs, stateA);

  compute_twopdm_sweep(solution, system, systemDot, newSystem, newEnvironment, big, numprocs, stateA);

  if (sweepParams.get_block_iter()  == sweepParams.get_n_iters() - 1)
    compute_twopdm_final(solution, system, systemDot, newSystem, newEnvironment, big, numprocs, stateA);

  SaveRotationMatrix (newSystem.get_sites(), rotateMatrix, stateA);

  //for(int i=0;i<dmrginp.nroots();++i)
  solution[0].SaveWavefunctionInfo (big.get_stateInfo(), big.get_leftBlock()->get_sites(), stateA);
  solution[0].deallocate();
  newSystem.transform_operators(rotateMatrix);

}

double SweepTwopdm::do_one(SweepParams &sweepParams, const bool &warmUp, const bool &forward, const bool &restart, const int &restartSize, int stateA, int stateB)
{
  Timer sweeptimer;
  int integralIndex = 0;
  ...
  save_twopdm_binary(twopdm, stateA, stateB); 
