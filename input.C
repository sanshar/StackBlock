...
SpinAdapted::Input::Input(const string& config_name) {
  ...
  if(rank == 0)
  {
	...
	m_lastM = atoi(tok[1].c_str());
      ...
      else if (boost::iequals(keyword,  "calcprocs")) {
	if (tok.size() < 2) {
	...
	m_thrds_per_node.resize(m_calc_procs.size(), atoi(tok[1].c_str()));
      }
      else if (boost::iequals(keyword, "memory") || boost::iequals(keyword, "mem")) {
	if (tok.size() != 3) {
	  pout << "keyword should be followed by a number (memory size) and either k, m or g"<<endl;
	  pout << "error found in the following line"<<endl;
	  pout << msg<<endl;
	  abort();
	}
	m_memory = atoi(tok[1].c_str());

	if (boost::iequals(tok[2], "k"))
	  m_memory *= 1e3/sizeof(double);
	else if (boost::iequals(tok[2], "m"))
	  m_memory *= 1e6/sizeof(double);
	else if (boost::iequals(tok[2], "g")) {
	  m_memory *= 1e9/sizeof(double);
	}
	else {
	  pout << "the units of memory should be either: k, m, g"<<endl;
	  abort();
	}
      }
  ...
  if (tok.size()==2 )
	...
	if(m_targetState == -1)
      ...
      else if (boost::iequals(keyword,  "responseaaac")) {
	if (tok.size() != 1) {
	  pout << "The keyword response should not be followed by anything!"<<endl;
	  abort();
	}
	m_calc_type = RESPONSEAAAC;
	m_solve_type = CONJUGATE_GRADIENT;
	if(m_targetState == -1)
	  m_targetState = 2;	
      }
...
//if (sym != "c1") // must be initialized even if c1 sym.
  ...
  for (int i=0; i<m_num_Integrals; i++) {
    ...
    mpi::broadcast(world, v_1[i].bin,0);
    mpi::broadcast(world, v_1[i].Occnum,0);

    mpi::broadcast(world, v_2[i].dim, 0);
    mpi::broadcast(world, v_2[i].indexMap, 0);
...
void SpinAdapted::Input::readorbitalsfile(string& orbitalfile, OneElectronArray& v1, TwoElectronArray& v2, double& coreEnergy, int integralIndex)
{
  ...
  if (rank == 0) {
    ...
    //TODO
    //Why is m_norbs rather htan m_norbs/2
...
void SpinAdapted::Input::readorbitalsfile(string& orbitalfile, OneElectronArray& v1, TwoElectronArray& v2, double& coreEnergy, PairArray& vcc, CCCCArray& vcccc, CCCDArray& vcccd, int integralIndex) {
...
//    i.second.ReSize(m_total_orbs*2,m_total_orbs*2,m_total_orbs*2,m_total_orbs*2);  
  ...
  mpi::broadcast(world,v1,0);
  mpi::broadcast(world,v2,0);
  mpi::broadcast(world,vpt1,0);
  mpi::broadcast(world,vpt2,0);
#endif

  long oneIntegralMem, twoIntegralMem, PerturboneIntegralMem;
  long twoedim = v2.matDim;
  if (RHF) { 
    oneIntegralMem = (m_total_orbs*(m_total_orbs+1))/2; 
    twoIntegralMem = twoedim*(twoedim+1)/2;
    PerturboneIntegralMem = (m_total_orbs*(m_total_orbs+1))/2; 
  }
  else { 
    oneIntegralMem = m_total_orbs*(m_total_orbs+1); 
    twoIntegralMem = twoedim*(twoedim+1)/2;
    PerturboneIntegralMem = m_total_orbs*(m_total_orbs+1); 
  }
...
#else
    //TODO
    //For MPS-SC-NEVPT, different MPS perturbers are calculated independently, through calling block through mpi. Block is serial by itself.  
  ...
  while(msg.size() != 0) {
    ...
    if (i==-1 && j==-1 && k==-1 && l==-1 && abs(value) < NUMERICAL_ZERO) {
      break;
    }
...
void SpinAdapted::Input::performSanityTest()
{
  ...
  */
  ...
  if ( m_calc_type==RESPONSEAAAC || m_calc_type==RESPONSEAAAV) {
    if (get_sweep_type() == FULL) {
      pout << "Responseaaac and Responseaaav should be used with partial sweep"<<endl;
      abort();
    }
  }
  for (int i=0; i<m_spin_orbs_symmetry.size(); i+=2) {
  ...
  if ((m_calc_type == RESPONSE || m_calc_type == RESPONSELCC || m_calc_type == RESPONSEAAAV || m_calc_type == RESPONSEAAAC || m_calc_type == RESPONSEBW )&& m_occupied_orbitals == -1) {
...
int SpinAdapted::Input::getHFQuanta(const StackSpinBlock& b) const
{
